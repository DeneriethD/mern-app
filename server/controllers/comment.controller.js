...
const addlikeComment = async (req, res) => {
    try {
      ...
      // Agrega el ID del usuario a la lista de "me gusta" del comentario
      const updatedComment = await Comment.findByIdAndUpdate(
        commentId,
        { $addToSet: { likes: userId } }, // Utilizamos $addToSet para evitar duplicados
        { new: true }
      );
  ...
  const addunlikeComment = async (req, res) => {
    try {
      ...
      // Elimina el ID del usuario de la lista de "me gusta" del comentario
      const updatedComment = await Comment.findByIdAndUpdate(
        commentId,
        { $pull: { likes: userId } },
        { new: true }
